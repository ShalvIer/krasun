...
const allowedMime =
  /^(image\/(jpeg|png|webp|gif)|audio\/(webm|ogg|mpeg|mp4|wav)|video\/(webm|mp4))$/;

const upload = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: Math.max(
      env.MAX_PHOTO_UPLOAD_BYTES,
      env.MAX_AUDIO_UPLOAD_BYTES,
      env.MAX_GIF_UPLOAD_BYTES,
      env.MAX_VIDEO_UPLOAD_BYTES
    ),
    files: 1
  },
...
async function storeFile(file: Express.Multer.File) {
  const storagePath =
    `${crypto.randomUUID()}${safeExtension(file.mimetype)}`;
...
uploadsRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!req.file) {
      throw new HttpError(400, "Choose an avatar file");
    }
    ...
    const previous = await prisma.mapAvatar.findUnique({
      where: { userId }
    });
    ...
    try {
      const avatar = await prisma.mapAvatar.upsert({
        ...
        update: {
          type,
          value: `/api/uploads/avatar/${storagePath}`,
          storagePath
        },
        create: {
          userId,
      ...
      if (
        ...
        await removeFile(previous.storagePath);
      }
  ...
  asyncRoute(async (req, res) => {
    ...
    const attachment =
      await prisma.messageAttachment.findUnique({
        where: {
          id: routeParam(req, "attachmentId")
        },
...
uploadsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const avatar = await prisma.mapAvatar.findFirst({
      where: { storagePath },
      include: { user: true }
    });
    ...
    if (!visible) {
      throw new HttpError(403, "Avatar access denied");
    }
