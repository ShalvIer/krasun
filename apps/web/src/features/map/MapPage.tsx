...
import { Crosshair, EyeOff, Lock, MapPin, MessageCircle, MoreHorizontal, Navigation, Pause, Play, Plus, Route, Satellite, X } from "lucide-react";
...
export function MapPage() {
  ...
  return <main className="map-page"><div ref={mapNode} className={mapToken ? "map-canvas" : "map-canvas no-token"}>{!mapToken && <div className="map-config"><Satellite /><h2>Mapbox token required</h2><p>Add <code>VITE_MAPBOX_TOKEN</code>. All map, marker and route code is ready.</p></div>}</div><header className="map-header"><div><p className="eyebrow">Group map</p><h1>{group.name}</h1></div><span className={`location-pill ${location.state.toLowerCase()}`}>{location.state}</span></header><div className="map-tools"><button onClick={centerOnSelf} aria-label="Center on me"><Crosshair /> <span>Me</span></button><button className={location.state === "LIVE" ? "active" : ""} aria-pressed={location.state === "LIVE"} onClick={location.start}>{location.state === "LIVE" ? <Pause /> : <Play />} <span>{location.state === "LIVE" ? "Sharing" : "Share"}</span></button><button className="spot-action" onClick={beginPlacing}><Plus /> <span>Spot</span></button><button onClick={() => setMoreOpen((open) => !open)} aria-expanded={moreOpen} aria-label="More map actions"><MoreHorizontal /> <span>More</span></button></div>
    {moreOpen && <aside className="map-more-sheet"><div><p className="eyebrow">Location controls</p><button onClick={() => setMoreOpen(false)} aria-label="Close"><X /></button></div><button onClick={() => { location.freeze(); setMoreOpen(false); }}><Pause /> Freeze current position</button><button onClick={() => { location.hide(); setMoreOpen(false); }}><EyeOff /> Hide from the map</button><button onClick={() => { void shareLocation(); setMoreOpen(false); }}><MessageCircle /> Share snapshot to chat</button></aside>}
